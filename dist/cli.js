 ...
 */
async function fetchAndCache(apiConfig) {
    try {
        const response = await fetch(apiConfig.input);
        const data = await response.json();
        const outputPath = path.resolve(process.cwd(), `${apiConfig.output}${apiConfig.generator.key}.json`);
        if (apiConfig.generator.src) {
            // Execute the custom generator script
            const customGeneratorOutput = execSync(`node ${apiConfig.generator.src}`).toString();
            await fs.writeFile(outputPath, customGeneratorOutput);
        }
        else {
            await fs.writeFile(outputPath, JSON.stringify(data, null, 2));
        }
        console.log(`API '${apiConfig.generator.key}' fetched and cached successfully.`);
    }
    catch (error) {
        console.error(`An error occurred while processing API '${apiConfig.generator.key}':`, error);
    }
}
/**
 ...
 */
export async function processApis(config) {
...
function runCLI() {
    ...
    program
        ...
        .option('-c, --config <path>', 'Path to the config file', './.cache-codegen.json')
